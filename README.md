...
フレーム・武器などのアイテムデータ（WFCD warframe-items）は、アプリの版ごとに決まった版を使う（表示にマウスを乗せると版が出る）。配布元の形式が予告なく変わっても取り込み結果が黙って変わらないようにするためで、代わりに**ゲームのアップデートで増えた新アイテムは、アプリを更新するまで取り込めない**。ドロップ表・Varziaの品揃え・シンジケート報酬は従来どおり最新を取得する。
