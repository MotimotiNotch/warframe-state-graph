...
- **情報に音量差をつける**: 操作頻度の低い要素（凡例、補足説明文等）は常時全文表示せず、
  ...
  面積を最小化する方向で作る。
- **ポップオーバー（`.popover`）は基本的に透過しない**: 背景パネル（`--panel`）は壁紙を
  透かす狙いで半透明だが、`.popover`は他要素の上に浮かぶ読み物なので専用の`--popover-bg`
  （ダーク`rgba(20, 22, 28, 0.94)`/ライト`rgba(255, 255, 255, 0.96)`、ほぼ不透明）を使うこと。
  `--panel`をそのまま流用すると背後の要素が透けて読みにくくなる。
